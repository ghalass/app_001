LoadTable();
const fichevieorgane = new Fichevieorgane();
const organe = new Organe();

function LoadTable() {
  $("#loaderTable").css("display", "block");
  get([], "fichevieorganes/getAll")
    .then((res) => {
      $("#div_table").html(res);
    })
    .then(() => {
...
$("form").submit(function (e) {
  ...
  const row = {
    ...
    dateinterv: formData.dateinterv,
  ...
  switch (formData.operation) {
    case "add":
      fichevieorgane.createOne(row).then((res) => {
        notif(res, "Ajouté");
      });
      break;
    case "update":
      fichevieorgane.updateOne({ by: "id", id: row.id }, row).then((res) => {
        notif(res, "Modifié");
      });
      break;
    case "delete":
      fichevieorgane.deleteOne({ by: "id", id: row.id }).then((res) => {
        notif(res, "Supprimé");
      });
      break;
    default:
...
$("#parc, #typeorgane").on("change", () => {
  const row = { parc: $("#parc").val(), typeorgane: $("#typeorgane").val() };
  // console.log(row);
  organe.findWhere(row).then((res) => {
    // console.log(res);
    let txt = `
    <option value=""></option>`;
